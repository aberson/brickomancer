...
- **Phase 3 Step 8** — React wizard wired to the rebuilt routes (`npm run build` clean); `tests/integration/test_smoke.py` exercises both paths + instructions against the real services (gated on `BRICKOMANCER_INTEGRATION=1`). Smoke PASSED: text 73 s, **image 1019 s (~17 min — one-time model load, cached since f471412)**, instructions 9 s.
