...
toolchain (Step 7/9) renders a cover page + BOM from a constant, never a
dynamically-generated header.  The frozen header is the artifact the harness is
forbidden to edit -- the v1 plateau postmortem traced ~30 commits of
meta-command oscillation (FADE_STEPS / HIGHLIGHT_STEP / COVER_PAGE / BOM churn)
to a freely-editable meta layer.  The "nothing else" assertions below are the
frozen-config guard that fails if that churn is ever reintroduced.
"""
...
def test_meta_header_contains_exactly_cover_bom_and_steps() -> None:
    """Header has exactly one COVER_PAGE, one BOM, and at least one step marker."""
    text = _META_HEADER.read_text(encoding="utf-8")
    assert text.count("0 !LPUB INSERT COVER_PAGE") == 1
    assert text.count("0 !LPUB INSERT BOM") == 1
    assert text.count("0 STEP") >= 1


def test_meta_header_has_nothing_else() -> None:
    """Frozen-config guard: the ONLY meta commands in the header are COVER_PAGE and BOM.

    The whitelist filters on the ``0 !`` prefix (ALL LDraw meta commands), not just
    ``0 !LPUB`` -- meta-churn can arrive as ``0 !FADE``, ``0 !SILHOUETTE``,
    ``0 !LEOCAD GROUP``, etc.  Any new ``0 !`` line that is not COVER_PAGE/BOM fails
    here, which is the regression this fixture exists to catch (v1 plateau:
    ~30 commits of FADE_STEPS/HIGHLIGHT_STEP/COVER_PAGE/BOM oscillation).
    """
    meta_lines = [ln for ln in _nonblank_lines(_META_HEADER) if ln.startswith("0 !")]
    assert meta_lines, "frozen header must contain at least the COVER_PAGE + BOM metas"
    for ln in meta_lines:
        assert ("INSERT COVER_PAGE" in ln) or ("INSERT BOM" in ln), (
...
@pytest.mark.parametrize("path", [_META_HEADER, _SMOKE])
def test_cover_page_after_a_step(path: Path) -> None:
    """COVER_PAGE placement invariant: it must follow a `0 STEP` (v1 hard-won fix)."""
    lines = _nonblank_lines(path)
    cover_idx = next(
        i for i, ln in enumerate(lines) if "INSERT COVER_PAGE" in ln
    )
    assert any(lines[j] == "0 STEP" for j in range(cover_idx)), (
        f"COVER_PAGE not preceded by a 0 STEP in {path.name}"
    )


@pytest.mark.parametrize("path", [_META_HEADER, _SMOKE])
def test_bom_after_last_step(path: Path) -> None:
    """BOM placement invariant: it must come after the final `0 STEP` (v1 fix)."""
    lines = _nonblank_lines(path)
