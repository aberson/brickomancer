...
*Generated by /build-phase on 2026-06-13. Append-only; re-running the phase adds new items below, never modifies existing ones.*

### M1: Diagnose rembg server-side sparsity
- **Source step:** Step 2 (operator, deferred)
- **Issue:** #34
- **Commands to run:**
  ```powershell
  # Set PATH and start the server (Step 1 must be merged first — it adds the fill log line)
  $env:PATH += ";C:\Tools\LPub3D"
  uv run uvicorn --app-dir src brickomancer.main:app

  # In a second terminal: POST a generate request
  curl.exe -X POST http://localhost:8000/api/generate/from-image `
    -F "image=@docs/example_input_output/star/input_image/cartoon_star2.png" `
    -F "height_studs=5"

  # Write verdict based on server stdout fill log
  # If fill < 5%:
  "SPARSE" | Out-File -FilePath docs/investigations/INV-7-step2-verdict -Encoding ascii
  # If fill 20-30%:
  "OK" | Out-File -FilePath docs/investigations/INV-7-step2-verdict -Encoding ascii
  ```
- **What you're looking for:**

  | Check | Expected outcome |
  |---|---|
  | Server stdout shows `silhouette fill:` line | Line present; fill % logged |
  | Fill % for cartoon_star2.png | < 5% → write SPARSE; 20–30% → write OK |
  | Verdict file created | `docs/investigations/INV-7-step2-verdict` exists |
  | Finding recorded in INV-7 | Observed fill % and diagnosis added to INV-7 doc |
