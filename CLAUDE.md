...
Steps 1–11 complete (2026-06-11); Harness Steps 12–18 complete + post-build fixes + two full harness runs (2026-06-13). Harness fully operational: 8/8 advisors (color_match replaces part_variety; reference_fidelity added using gold dataset star), weighted developer-agent loop, pytest gate (unit tests only), avg-raw quality gate (≥ 8.0). Top-surface bricks tile-smoothed via `_apply_surface_tiles()` (TILE_PART_IDS in brick.py). Desktop launcher at `scripts/run_harness.bat`. `/run-harness` skill for one-command prep + launch + monitoring. 311 unit tests passing, 0 type errors, 0 lint violations. Gold dataset: `docs/example_input_output/star/` (cartoon_star.jpg + cartoon_star2.png inputs + 10 gold step PNGs); harness uses `_HEIGHT_STUDS = 5` (matches gold reference ~5-high star). Server restarts after each PASS_COMMITTED so code changes take effect within the same run.

**Harness run 2 commits (2026-06-13):** 8bd95b3 axis transpose (star face → XZ plane), 81fd8e2 tile Y-coordinate fix (flush on studs), bd1b576 subject-color filter (yellow star over white background).
